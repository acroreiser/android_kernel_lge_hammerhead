...
{
	if (!ext4_valid_contents_enc_mode(ci->ci_data_mode) || !ext4_valid_filenames_enc_mode(ci->ci_filename_mode)) {
...
#define hlist_entry_safe_compat(ptr, type, member) \
	(ptr) ? hlist_entry(ptr, type, member) : NULL

#define hlist_for_each_entry_compat(pos, head, member)				\
	for (pos = hlist_entry_safe_compat((head)->first, typeof(*(pos)), member);\
	     pos;							\
	     pos = hlist_entry_safe_compat((pos)->member.next, typeof(*(pos)), member))

#define hash_for_each_possible_compat(name, obj, member, key)			\
	hlist_for_each_entry_compat(obj, &name[hash_min(key, HASH_BITS(name))], member)
			  ...
			  const struct ext4_crypt_info *ci)
{
	...
	hash_for_each_possible_compat(ext4_crypt_master_keys, mk, mk_node, hash_key) {
